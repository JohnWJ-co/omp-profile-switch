...
- **子代理（task 派生）会话跳过**：子代理的 session header 带 `parentSession` 标记，自动对齐跳过它们，让其自身 `task.agentModelOverrides` / agent 定义指定的模型生效，不被档案默认模型覆盖

这样每次切档案后，主模型在新会话里都能稳定落在档案默认上，且子代理模型配置保持独立。
